...
#pragma acc parallel loop gang worker num_workers(8) vector_length(128) async(2+k)
                for (int i = p * b; i < (p + 1) * b; i++) {
#pragma acc loop vector
...
#pragma acc parallel loop gang worker num_workers(4) vector_length(128) async(3+k)
                for (int j = p * b; j < (p + 1) * b; j++) {
#pragma acc loop vector
                    for (int i = 0; i < p * b; i++) {
                        float tp = data[index(i, k)] + data[index(k, j)];
...
#pragma acc parallel loop gang worker num_workers(4) vector_length(128) async(4+k)
                for (int j = p * b; j < (p + 1) * b; j++) {
#pragma acc loop vector
