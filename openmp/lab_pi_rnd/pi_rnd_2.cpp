...
static long MULTIPLIER2 = 1366;
static long ADDEND2 = 150889;
static long PMOD2 = 714025;
...
int main() {
    ...
    double r = 1.0;
    int num_in = 0;
#pragma omp parallel num_threads(30) reduction(+ : num_in)
    {
        // 使用一种随机数生成算法生成若干随机数种子
#pragma omp single
        {
            ...
            for (int i = 1; i < nthreads; ++i) {
                iseed = rand() % PMOD1;  //(unsigned long long)((MULTIPLIER1 *
                                         //iseed) % PMOD1);
                pseed[i] = iseed;
            }
        }
        // 每个线程依据上面的随机数，使用其他随机数生成算法进行生成
