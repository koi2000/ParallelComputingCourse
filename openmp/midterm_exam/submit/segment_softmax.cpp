...
int main() {
    ...
    // memcpy(&datas[0], &data[0], SIZE * sizeof(double));

    vector<int> flags= vector(flag.begin(), flag.end());
    // memcpy(&flags[0], &flag[0], SIZE * sizeof(double));

    vector<int> tpflags = vector(flag.begin(), flag.end());
    // memcpy(&tpflags[0], &flag[0], SIZE * sizeof(double));
    ...
    for (int i = 1; i < SIZE; i++) {
        if (flag[i]) {
            sums.push_back(datas[i - 1]);
        }
    }
    sums.push_back(datas[SIZE - 1]);
    int idx = 0;
    for (int i = 0; i < SIZE; i++) {
    ...
    for (int i = 0; i < SIZE; i++) assert(abs(data[i] - test[i]) < 1e-5);
}
